...
public class DAX_CameraControl_V2_1 : MonoBehaviour  
{
	...
	void Awake()
	{
		GameObject MCT = GameObject.Find( "MainCameraTransform" ) as GameObject;
		if ( MCT!=null)
		{
			if ( MCT.activeSelf )
			{
				this.gameObject.transform.rotation = MCT.transform.rotation;	
				this.gameObject.transform.position = MCT.transform.position;	
			}	
		}
	}
